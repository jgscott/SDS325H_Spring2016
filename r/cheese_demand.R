...
# A different way: work on the log scale
...
# Confidence interval for offset on the log scale
boot1 = do(1000)*{
	lm(log(vol)~disp, data=resample(cheese))
}
confint(boot1)

# Additive change in volume on log scale means multiplicative change on original scale
# Therefore exponentiate dummy-variable coefficient to get percentage change.
exp(confint(boot1$disp))

# Compare the earlier answer: they don't line up
confint(boot_mean$X1)

# Why? The mean on the original scale is heavily influenced
# by the long right tail.
hist(cheese$vol, 100)

# The mean on the log scale is more stable.
hist(log(cheese$vol), 100)

# However, each of these analyses misses something... look store by store
...
# We can account for this using a model: put in dummy variables store by store.
